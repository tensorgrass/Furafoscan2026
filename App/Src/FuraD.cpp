...
void FuraD::main() {
  switch (current_step) {
    ...
    case enum_step_fura::STEP_DEFENSE__STEP2_ATTACK:
      ...
      detectDistanceCenterOut(enum_step_fura::STEP_DISTANCE_BOTH_DETECTED, F_D_DEFENSE_ATTACK_OUT_MS);
      if (current_step != previous_step) break;
      break;
...
//        setMotorSpeedRamp(F_D_ESC_BASE_SPEED_RAMP_INI, F_D_DISTANCE_LATERAL_SPEED_LEFT, F_D_ESC_BASE_SPEED_RAMP_INI, F_D_DISTANCE_SPEED_SLOW_RIGHT);
    ...
    case enum_step_fura::STEP_DISTANCE_CENTER_BOOST:
      ...
      detectDistanceCenterOut(enum_step_fura::STEP_DISTANCE_BOTH_DETECTED, F_D_DISTANCE_BOTH_BOOST_OUT_MS);
      if (current_step != previous_step) break;
      break;
...
// Se ha empezado a detectar al oponente
    #ifdef F_D_ACTIVE_LED_DISTANCE_SENSOR_CENTER
    if (controller->getLedDistanceCenter()->get_status() == 0) controller->getLedDistanceCenter()->turn_on();
    #endif
    if (distance_center_in__time_tick_ini == 0) {
      ...
      if (distance_center_in__time_tick_current >= F_D_DISTANCE_CENTER_BOOST_MS) {
        // Oponente detectado descartando rebotes
        distance_center_in__time_tick_ini = 0;
        goNextStep(next_step);
        return;
